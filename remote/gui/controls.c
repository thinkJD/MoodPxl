...
const char* const TXTKeys[10] = 
	{TXTKey1,TXTKey2,TXTKey3,TXTKey4,TXTKey5,TXTKey6,TXTKey7,TXTKey8,TXTKey9,TXTKey0};
...
void trackbar_draw (TRACKBAR* trackbar, unsigned char sel) {
	unsigned char c;
	
	//Schriftart einstellen
	set_font (trackbar->font);

	//Wenn die Trackbar disabled ist, wird sie ausgegraut gezeichnet
	if(trackbar->type & BC_Disabled)
		c=2;
	else
		c=3;
	
	//trackbar zeichnen 
	draw_rect (trackbar->left+10, trackbar->top,trackbar->width + trackbar->left - 30 ,
		trackbar->height , 1 ,c,DRAW_PUT);
	draw_rect (trackbar->left+10+2, trackbar->top+2,trackbar->width + trackbar->left-30-4, 
		trackbar->height-4, 1 ,c,DRAW_PUT);
	
	//Label zeichnen links neben der trackbar und auf der Hälfte der Höhe
	draw_string (trackbar->left, trackbar->top + trackbar->height/2 - 4, trackbar->caption, 
		c, DRAW_PUT);
	
	//Spielen mit DRAW_XOR evtl. lässt sich der ValueString so in die trackbar schreiben	
	
	if (sel)
	{
		draw_rect (trackbar->left+10+1, trackbar->top+1,trackbar->width + trackbar->left-30-2, 
			trackbar->height-2, 1 ,c,DRAW_PUT);
	}
	else
	{
		draw_rect (trackbar->left+10+1, trackbar->top+1,trackbar->width + trackbar->left-30-2, 
			trackbar->height-2,1 ,c,DRAW_ERASE);
	}
	
	//Positon des Cursors frei machen 
	_draw_block(trackbar->left+10+3, trackbar->top+3, trackbar->width + trackbar->left-30-6,
		trackbar->height-6, c, DRAW_ERASE);
	//Cursor zeichnen
	unsigned char offset = (trackbar->width-36) * trackbar->actval / trackbar->maxval;
	if (offset > (trackbar->width-36)) offset = trackbar->width-36; 
	_draw_block(trackbar->left+10+3+offset, trackbar->top+3, 1 , trackbar->height-6, c, DRAW_PUT);
	
 	if (offset <= 4)
 	{
 		draw_arrow (trackbar->left+10+4+offset, trackbar->top+5, ARROW_RIGHT, 4, 
 			LCD_COLOR_B, DRAW_PUT);
  	}
 
    if (offset >= (trackbar->width + trackbar->left-30-9))
    {
  		draw_arrow (trackbar->left+9+offset, trackbar->top+5, ARROW_LEFT, 4, LCD_COLOR_B, DRAW_PUT);
  	}
  	
  	if ((offset > 4) && (offset < (trackbar->width + trackbar->left-30-9)))//
  	{
 	draw_arrow (trackbar->left+10+4+offset, trackbar->top+5, ARROW_RIGHT, 4, LCD_COLOR_B, DRAW_PUT);
 	draw_arrow (trackbar->left+9+offset, trackbar->top+5, ARROW_LEFT, 4, LCD_COLOR_B, DRAW_PUT);
 	}
 	
	//Raum für Value freimachen
	_draw_block(trackbar->left+trackbar->width-20, trackbar->top, 30,
		trackbar->height, c, DRAW_ERASE);
	//Value zeichnen
	draw_numU(trackbar->left+trackbar->width-15, trackbar->top + 
		trackbar->height/2 - 4,trackbar->actval,0,c,DRAW_PUT);
...
void control_draw (CONTROL* control, unsigned char select) {
...

	{
		...
		case BC_Listbox:
			...
			break;
		case BC_Trackbar:
			trackbar_draw((TRACKBAR*)control,select);
			break;
	}
}	// control_draw
...
void trackbar_changevalue (TRACKBAR* trackbar) {
	if((KEY_Left) && (trackbar->actval!=0))
		trackbar->actval--;
	if((KEY_Right) && (trackbar->actval < trackbar->maxval))
		trackbar->actval++;
}
		...
		case BC_Trackbar:
			{
				...
				if((KEY_Left) || (KEY_Right))
					trackbar_changevalue(trackbar);
				if((x!=trackbar->actval) && (trackbar->ONchange))
					trackbar->ONchange(form,control);
			}
			break;
	}
